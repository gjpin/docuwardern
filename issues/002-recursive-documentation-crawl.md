...
Extend `docuwarden scrape` to recursively discover in-scope documentation pages from every HTML `a[href]`. Convert each accepted page into the same corpus artifact established by the single-page workflow.
...
- Every anchor is considered without selector configuration.
